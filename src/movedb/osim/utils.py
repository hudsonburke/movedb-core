...
from pyopensim.simulation import Model, ForceSet, CustomJoint, FreeJoint, Coordinate, SetCoordinates
from pyopensim.tools import CMC_TaskSet, CMC_Joint
from pyopensim.actuators import CoordinateActuator, PointActuator, TorqueActuator
import os
...
def createActuatorsFile(modelpath=None):
    ...
    for iCoord in range(nCoord):
        ...
        if parentName == model.getGround().getName():
            ...
            if (joint.getConcreteClassName() == 'CustomJoint' or 
                joint.getConcreteClassName() == 'FreeJoint'):
                
                motion = str(coordinate.getMotionType())
                
                # Get coordinate transform axis
                if joint.getConcreteClassName() == 'CustomJoint':
                    concreteJoint = CustomJoint.safeDownCast(joint)
                else:
                    concreteJoint = FreeJoint.safeDownCast(joint)
                sptr = concreteJoint.getSpatialTransform()
                for ip in range(6):
                    if str(sptr.getCoordinateNames().get(ip)) == str(coordinate.getName()):
            ...
            newActuator = CoordinateActuator(str(coordinate.getName()))
...
def createCMCTaskSet(model_path):
    ...
    coord_set : SetCoordinates = model.getCoordinateSet()
    nCoord = coord_set.getSize()

    # Add a CMC_Joint to the TaskSet for each coordinate
    for i in range(nCoord):
        coord : Coordinate = coord_set.get(i)
...
def _find_platform_for_forceplate_number(fp_number: int, forceplate_names: list[str]) -> int | None:
    """
