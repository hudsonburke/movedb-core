...
class MarkerGapResult:
    ...
    def gap_percentage(self) -> float:
        ...
        if self.duration.total_seconds() == 0:
            return 0.0
        return (self.total_gap_duration.total_seconds() / self.duration.total_seconds()) * 100
...
def detect_marker_gaps(marker: Marker) -> MarkerGapResult:
    ...
    total_duration = df.select(pl.col("timestamp").max() - pl.col("timestamp").min()).item()
