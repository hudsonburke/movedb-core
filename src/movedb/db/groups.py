...
class CaptureSessionGroup(SQLModel, table=True):
    ...
    capture_sessions: list["CaptureSession"] = Relationship(
        back_populates="groups", link_model=CaptureSessionGroupLink
    )
...
class SubjectGroup(SQLModel, table=True):
    ...
    subjects: list["Subject"] = Relationship(
        back_populates="groups", link_model=SubjectGroupLink
    )
...
class TrialGroup(SQLModel, table=True):
    ...
    trials: list["Trial"] = Relationship(
        back_populates="groups", link_model=TrialGroupLink
    )
